...
```bash
volatility2 -f "$dump_file" --profile="$profile" memdump -p "$pid" -D "$output_dir"
```
...
```bash
volatility2 -f "$dump_file" --profile="$profile" procdump -p "$pid" -D "$output_dir"
```
...
```bash
volatility2 -f "$dump_file" --profile="$profile" yarascan -y "$rule_file"
```
